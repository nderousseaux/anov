...
test.afterEach(async () => {
  ...
  try {
    const response = await fetch(
      "http://localhost:3000/api/admin/reservations/cleanup",
      {
        method: "DELETE",
      },
    );
...
test.describe("API /api/reservations/availability", () => {
  test("Jour normal avec disponibilité", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/reservations/availability?date=${dateStr}&guests=2`,
    );
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
  ...
  test("Jour fermé (lundi - jour de fermeture par défaut)", async ({
    request,
  }) => {
  ...
  test("Month query - liste des dates indisponibles", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/reservations/availability?month=${monthStr}&guests=2`,
    );
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
  ...
  test("Erreur - Guests = 0", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/reservations/availability?date=${dateStr}&guests=0`,
    );
    expect(response.status()).toBe(400);
    const data = await response.json();
  ...
  test("Erreur - Guests = 5", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/reservations/availability?date=${dateStr}&guests=5`,
    );
    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.error).toBe("guests invalide (1 à 4)");
  ...
  test("Erreur - Guests invalide (abc)", async ({ request }) => {
    ...
    const response = await request.get(
      `/api/reservations/availability?date=${dateStr}&guests=abc`,
    );
    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.error).toBe("guests invalide (1 à 4)");
  ...
  test("Erreur - Date format invalide", async ({ request }) => {
    const response = await request.get(
      `/api/reservations/availability?date=2026/07/15&guests=2`,
    );
    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.error).toBe("date invalide");
...
test.describe("API /api/reservations/[id] - Get by ID", () => {
  ...
  test("Réservation existante", async ({ request }) => {
    ...
    const response = await request.get(
      "/api/reservations/res_test_non_existant",
    );
  ...
  test("Réservation non trouvée (404)", async ({ request }) => {
    const response = await request.get(
      "/api/reservations/non_existent_id_12345",
    );
    expect(response.status()).toBe(404);
  });
});
...
test.describe("API /api/reservations/cancel - Annulation", () => {
  ...
  test("Erreur - Token invalide (404)", async ({ request }) => {
    const response = await request.get(
      "/api/reservations/cancel?token=token_invalide_12345",
    );
    expect(response.status()).toBe(404);
    const data = await response.json();
...
test.describe("Processus de réservation complet", () => {
  test("Flow complet: Formulaire → Stripe → Confirmation", async ({ page }) => {
    ...
    const dayButtons = await page
      .locator('button[type="button"][role="gridcell"]')
      .all();
    ...
    const availableSlots = page.locator(
      'button:has-text("12:00"):not([disabled]), button:has-text("19:00"):not([disabled])',
    );
    ...
    if (count > 0) {
      ...
      const allSlots = page.locator(
        'button:has-text("12:00"), button:has-text("19:00")',
      );
  ...
  test("Resto complet (409) - Tentative de réservation impossible", async ({
    request,
  }) => {
...
test.describe("Edge Cases - Réservation", () => {
  ...
  test("Date du lendemain avec tous les créneaux disponibles", async ({
    request,
  }) => {
