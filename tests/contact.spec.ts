...
test.describe("Formulaire de Contact - Page", () => {
  ...
  test("Affiche les informations de contact", async ({ page }) => {
    ...
    const contactInfo = page.locator(
      "#contact .grid.grid-cols-2, #contact .lg\\:flex",
    );
  ...
  test("Valide la longueur maximale des champs", async ({ page }) => {
    ...
    await expect(
      page.getByText("Un ou plusieurs champs dépassent la longueur maximale"),
...
test.describe("Formulaire de Contact - Envoi d'emails", () => {
  test("Envoie les deux emails (admin et confirmation) avec Mailcatcher", async ({
    page,
  }) => {
    await page.goto("/?lang=fr");
    await waitForSplashScreenToFade(page);
    ...
    await page
      .locator("#message")
      .fill(
        "Bonjour, je voudrais savoir si vous avez des options végétariennes.",
      );
    ...
    const adminEmail = emails.find(
      (e: any) =>
        e.subject.includes("Question sur la carte") ||
    ...
    expect(confirmationEmail?.recipients[0]).toContain(
  ...
  test("Envoie les deux emails avec vérification du contenu", async ({
    page,
  }) => {
    await page.goto("/?lang=fr");
    await waitForSplashScreenToFade(page);
    ...
    await page
      .locator("#message")
      .fill(
        "Nous sommes 10 personnes et nous souhaitons réserver pour un événement special.",
      );
    ...
    const adminEmail = emails.find(
      (e: any) =>
        e.subject.includes("[Contact") ||
        e.subject.includes("Informations groupe"),
    );
    expect(adminEmail).toBeDefined();
    // Mailcatcher retourne les recipients avec <email>
    expect(adminEmail?.recipients).toEqual(["<contact@anovrestaurant.fr>"]);
  ...
  test("Envoi fonctionne pour toutes les langues", async ({ page }) => {
    ...
    const adminEmail = emails.find(
      (e: any) =>
        e.subject.includes("Anfrage") || e.subject.includes("[Contact"),
    );
    expect(adminEmail).toBeDefined();
  });
});
...
test.describe("Formulaire de Contact - Responsive", () => {
  ...
  test("Affiche correctement sur desktop", async ({ page }) => {
    ...
    const contactGrid = page.locator(
      "#contact .grid.grid-cols-2, #contact .lg\\:flex",
    );
...
test.describe("Formulaire de Contact - Intégration complète", () => {
  test("Envoie de message complet avec vérification Mailcatcher", async ({
    page,
  }) => {
    ...
    const message =
      "Bonjour, nous organisons un événement corporatif pour 25 personnes et aimerions obtenir un devis pour une formule sur mesure. Quelle est votre capacité maximum pour les événements privés ?";
    ...
    const adminEmail = emails.find(
      (e: any) =>
        ...
        e.recipients.some((r: string) =>
          r.includes("contact@anovrestaurant.fr"),
        ),
    );
    expect(adminEmail).toBeDefined();
    ...
    if (adminEmail?.id) {
      ...
      expect(html).toContain(
        "Bonjour, nous organisons un événement corporatif",
      );
    }
    ...
    const confirmationEmail = emails.find(
      (e: any) =>
        Array.isArray(e.recipients) &&
        e.recipients.some((r: string) =>
          r.includes("jeanpaul.citoyen@villeparis.fr"),
        ) &&
        e.subject.toLowerCase().includes("message reçu"),
    );
    expect(confirmationEmail).toBeDefined();
  ...
  test("Envoie multiple messages consécutifs", async ({ page }) => {
    ...
    const hasClient1 = emails.some(
      (e: any) =>
        Array.isArray(e.recipients) &&
        e.recipients.some((r: string) => r.includes("client1@test.com")),
    );
    const hasClient2 = emails.some(
      (e: any) =>
        Array.isArray(e.recipients) &&
        e.recipients.some((r: string) => r.includes("client2@test.com")),
    );
