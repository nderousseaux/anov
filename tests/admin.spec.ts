...
test.describe("Interface Admin - Authentification", () => {
  ...
  test("Connexion réussie avec identifiants valides", async ({ page }) => {
    ...
    await expect(page.locator('button[type="button"]')).toHaveCount(35, {
      timeout: 10000,
    });
  });
  ...
  test("Connexion échouée avec identifiants invalides", async ({ page }) => {
    ...
    await expect(
      page.locator('p:text("Identifiants invalides")'),
    ).toBeVisible();
...
test.describe("Interface Admin - Navigation", () => {
  ...
  test('Le lien "Réservations" est actif par défaut', async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Réservations" })).toHaveClass(
      /bg-primary\/15/,
      { timeout: 5000 },
    );
    ...
    ).not.toHaveClass(/bg-primary\/15/);
    await expect(page.getByRole("link", { name: "CMS" })).not.toHaveClass(
      /bg-primary\/15/,
    );
  });

  test("La navigation vers la page Bons Cadeaux fonctionne", async ({
    page,
  }) => {
    // Se connecter explicitement
    await login(page);
  ...
  test("La navigation vers la page Réservations fonctionne depuis Bons Cadeaux", async ({
    page,
  }) => {
    // Se connecter explicitement
    await login(page);
  ...
  test("La navigation vers la page Réservations fonctionne depuis CMS", async ({
    page,
  }) => {
    ...
    await page.waitForURL(/\/admin\/login\?next=.*keystatic.*/, {
      timeout: 60000,
    });
...
test.describe("Interface Admin - Page Réservations", () => {
  ...
  test("La page Réservations affiche la légende du titre", async ({ page }) => {
    await expect(
      page.locator('p:text("Gestion des réservations et disponibilités")'),
    ).toBeVisible();
  });

  test("La page Réservations affiche le bouton Paramètres", async ({
    page,
  }) => {
    await expect(
      page.getByRole("button", { name: "Paramètres" }),
    ).toBeVisible();
  });
  ...
  test("Le bouton Paramètres affiche le panneau de configuration", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('label:text("Jours d\'ouverture")'),
    ).toBeVisible();
    await expect(
      page.locator('label:text("Acompte par couvert")'),
    ).toBeVisible();
    await expect(page.locator('label:text("Durée d\'un repas")')).toBeVisible();
    await expect(
      page.locator('label:text("Horaires d\'ouverture")'),
    ).toBeVisible();
  });

  test("La page Réservations affiche la navigation du calendrier", async ({
    page,
  }) => {
...
test.describe("Interface Admin - Page Bons Cadeaux", () => {
  ...
  test("La page Bons Cadeaux affiche le bouton Créer un bon cadeau", async ({
    page,
  }) => {
    await expect(
  ...
  test("La page Bons Cadeaux affiche le bouton Rafraîchir", async ({
    page,
  }) => {
    await expect(
  ...
  test("La page Bons Cadeaux affiche les cartes de statistiques", async ({
    page,
  }) => {
  ...
  test("Le bouton Créer un bon cadeau ouvre le modal", async ({ page }) => {
    ...
    await expect(
      page.locator('label:text("Email du destinataire")'),
    ).toBeVisible();
    await expect(page.locator("input#recipientEmail")).toBeVisible();
    await expect(
      page.locator('label:text("Message personnalisé")'),
    ).toBeVisible();
...
test.describe("Interface Admin - Navigation Responsive", () => {
  test("La navigation s'affiche correctement sur desktop (1920x1080)", async ({
    page,
  }) => {
  ...
  test("La navigation s'affiche correctement sur tablette (768x1024)", async ({
    page,
  }) => {
  ...
  test("La navigation s'affiche correctement sur mobile (375x667)", async ({
    page,
  }) => {
  ...
  test("La navigation s'affiche correctement sur mobile large (414x896)", async ({
    page,
  }) => {
...
test.describe("Interface Admin - Redirection non connecté", () => {
  ...
  test("Redirection vers la page de login si non connecté (Bons Cadeaux)", async ({
    page,
  }) => {
  ...
  test("Redirection vers la page de login si non connecté (CMS)", async ({
    page,
  }) => {
