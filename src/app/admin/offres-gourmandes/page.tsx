...
import { GourmetOfferStats } from "@/components/admin/GourmetOfferStats";
import type { GourmetOfferStats as GourmetOfferStatsType } from "@/components/admin/types";
import { GourmetOfferCard } from "@/components/admin/GourmetOfferCard";
import { GourmetOfferFilters } from "@/components/admin/GourmetOfferFilters";
import { RefreshCw, Plus, Mail, X } from "lucide-react";

interface GourmetOffer {
  id: string;
  code: string;
  offerName: string;
  price: number;
...
interface GourmetOfferListResponse {
  data: GourmetOffer[];
  total: number;
  page: number;
  pageSize: number;
...
function GourmetOfferListFallback() {
  return (
...
function GourmetOfferPageContent() {
  const [gourmetOffers, setGourmetOffers] = useState<GourmetOffer[]>([]);
  const [stats, setStats] = useState<GourmetOfferStatsType>({
    totalIssued: 0,
    totalAmount: 0,
    active: 0,
    expired: 0,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const listResponse = await fetch(
        `/api/admin/gourmet-offers?${params.toString()}`,
      );
      if (!listResponse.ok)
        throw new Error("Erreur lors du chargement des offres gourmandes");

      const listData: GourmetOfferListResponse = await listResponse.json();
      setGourmetOffers(listData.data);

      // Fetch stats
      const statsResponse = await fetch("/api/admin/gourmet-offers/stats");
      if (!statsResponse.ok)
        throw new Error("Erreur lors du chargement des statistiques");

      const statsData: GourmetOfferStatsType = await statsResponse.json();
      setStats(statsData);
    } catch (err) {
  ...
  const handleValidateGourmetOffer = async (id: string) => {
    try {
      const response = await fetch("/api/admin/gourmet-offers", {
        method: "PATCH",
        headers: {
  ...
  const handleMarkUsed = (gourmetOffer: GourmetOffer) => {
    // If expiresAt is null, treat it as not expired
    const now = new Date();
    let isExpired = false;
    if (gourmetOffer.expiresAt) {
      const expiresAt = new Date(gourmetOffer.expiresAt);
      isExpired = now > expiresAt;
    }

    if (isExpired) {
      const formattedDate = gourmetOffer.expiresAt
        ? formatDateTime(gourmetOffer.expiresAt)
        : "";
      if (
        !confirm(
          `Attention, cette offre gourmande est expirée depuis le ${formattedDate}. Êtes-vous sûr de vouloir la marquer comme utilisée ?`,
        )
      ) {
        return;
      }
    } else if (
      !confirm("Êtes-vous sûr de vouloir marquer cette offre comme utilisée ?")
    ) {
      return;
    }

    handleMarkUsedAction(gourmetOffer.id);
  };

  // Wrapper function for GourmetOfferCard component (only passes required props)
  ...
  // La suppression d'offres gourmandes est désactivée
  const handleDeleteGourmetOffer = () => {
    setError("La suppression d'offres gourmandes n'est pas autorisée.");
  };
  ...
  const handleCreateGourmetOffer = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!createFormData.offerName.trim()) {
      setError("Le nom de l'offre est requis");
      return;
    }

    if (!createFormData.price) {
    ...
    try {
      const response = await fetch("/api/admin/gourmet-offers", {
        ...
        body: JSON.stringify({
          offerName: createFormData.offerName,
      ...
      const newGourmetOffer = await response.json();
      ...
      if (window) {
        alert(
          `Code créé avec succès : ${newGourmetOffer.code}\n\nCe code a été créé depuis l'interface admin.\n${newGourmetOffer.recipientEmail ? `Email envoyé à : ${newGourmetOffer.recipientEmail}` : "Aucun email envoyé - le code doit être partagé manuellement."}`,
        );
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Une erreur est survenue");
    ...
    <div className="min-h-screen bg-background text-foreground overflow-y-auto">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Gestion complète des offres gourmandes
            </p>
          </div>
        ...
        <Suspense fallback={<StatsFallback />}>
          <GourmetOfferStats stats={stats} />
        </Suspense>

        {/* Filters */}
        <div className="mb-6">
          <GourmetOfferFilters filters={filters} setFilters={setFilters} />
        </div>
        ...
        {/* Gourmet Offer Grid */}
        {loading && !gourmetOffers.length ? (
          <GourmetOfferListFallback />
        ) : (
          <>
              ...
              <div className="text-center py-16">
                <div className="bg-card border border-border rounded-lg p-8 inline-block">
                  <h3 className="text-xl font-semibold text-foreground mb-2">
                    Aucune offre gourmande
                  </h3>
                  <p className="text-muted-foreground">
                    Les offres gourmandes seront disponibles prochainement
                  </p>
                </div>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {gourmetOffers.map((gourmetOffer) => (
                  <GourmetOfferCard
                    key={gourmetOffer.id}
                    ...
                    onValidate={handleValidateGourmetOffer}
                    onMarkUsed={handleMarkUsedForCard}
                    onDelete={handleDeleteGourmetOffer}
                  />
                ))}
              </div>
            )}
      ...
      {/* Modal de création d'offre gourmande */}
      {isCreateModalOpen && (
        <div
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleCreateGourmetOffer} className="space-y-4">
              {/* Nom de l'offre */}
              <div>
                <label
                  htmlFor="offerName"
                ...
                <input
                  ...
                  type="text"
                  required
                  value={createFormData.offerName}
                  onChange={(e) =>
                  ...
                  className="w-full px-3 py-2 bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-gold focus:border-transparent"
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={!createFormData.offerName || !createFormData.price}
                  className={`flex-1 px-4 py-2 font-semibold rounded-lg transition-colors ${
                    createFormData.offerName && createFormData.price
...
export default function AdminGourmetOffersPage() {
  return (
    <Suspense fallback={<StatsFallback />}>
      <GourmetOfferPageContent />
    </Suspense>
  );
}
