...
test.describe("CMS Content Verification - Homepage", () => {
  ...
  // --- Hero Section Tests ---
  test("Hero: Affiche le contenu depuis content/hero.yaml", async ({
    page,
  }) => {
  ...
  test("History: Affiche le contenu depuis content/histoire.yaml", async ({
    page,
  }) => {
    const histoireYamlPath = path.join(
      __dirname,
      "..",
      "content",
      "histoire.yaml",
  ...
  test("Contact: Affiche le contenu depuis content/contact.yaml", async ({
    page,
  }) => {
    const contactYamlPath = path.join(
      __dirname,
      "..",
      "content",
      "contact.yaml",
    ...
    if (address) {
      ...
      await expect(
        page.locator("#contact p").filter({ hasText: "Paris" }),
      ).toHaveText(/Paris/);
    }
    ...
    if (email) {
      await expect(page.locator('#contact a[href^="mailto:"]')).toHaveText(
        email,
      );
    }
  });
  ...
  test("Gallery: Vérifie le contenu YAML", async ({ page }) => {
    const galerieYamlPath = path.join(
      __dirname,
      "..",
      "content",
      "galerie.yaml",
    ...
    const photos = galerieContent.photos as
      Array<{ image?: string; caption_fr?: string }> | undefined;
  ...
  // --- Origins Map Tests ---
  test("Origins Map: Affiche le contenu depuis content/origines.yaml", async ({
    page,
  }) => {
    const originesYamlPath = path.join(
      __dirname,
      "..",
      "content",
      "origines.yaml",
...
test.describe("CMS Content Verification - Multilingue", () => {
  ...
  async function verifyAllContentForLanguage(
    page: any,
    locale: "fr" | "en" | "de",
  ) {
    ...
    if (histoireYaml[introTitleKey]) {
      await expect(page.locator("#history h2").first()).toHaveText(
        histoireYaml[introTitleKey],
      );
    }
    if (histoireYaml[introText1Key]) {
      await expect(page.locator("#history p").first()).toHaveText(
        histoireYaml[introText1Key],
      );
    }
    if (histoireYaml[introText2Key]) {
      await expect(page.locator("#history p").nth(1)).toHaveText(
        histoireYaml[introText2Key],
      );
    }
  }
...
test.describe("CMS Content Verification - Full Page Content Check", () => {
  test("Page complète: Tous les contenus YAML s'affichent", async ({
    page,
  }) => {
    await page.goto("/?lang=fr");
    await waitForSplashScreenToFade(page);
