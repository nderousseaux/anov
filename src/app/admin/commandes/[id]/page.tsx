    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto p-6">
        <Link href="/admin/commandes" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><path d="m15 18-6-6 6-6"/></svg> Retour aux commandes
        </Link>

        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Commande {order.code}</h1>
            <p className="text-sm text-muted-foreground">{new Date(order.createdAt).toLocaleString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
          </div>
          <span className={`px-4 py-2 rounded-full text-sm font-medium ${
            ...
            {order.status === 'PENDING_PAYMENT' ? 'En paiement...' :
             order.status === 'CONFIRMED' ? 'En attente...' :
             order.status === 'PROCESSING' ? 'En préparation' :
             order.status === 'SHIPPED' ? 'Envoyée' :
             order.status === 'READY' ? 'Prête' :
             order.status === 'COMPLETED' ? 'Terminée' :
             order.status === 'CANCELLED' ? 'Remboursée' : 'Expirée'}
          </span>
        </div>
