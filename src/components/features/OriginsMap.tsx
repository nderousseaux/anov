...
export function OriginsMap({ content }: OriginsMapProps) {
  ...
  const rayMapRef = useRef<
    Map<string, d3.Selection<SVGLineElement, unknown, null, undefined>>
  >(new Map());
  ...
  const p = useCallback(
    (key: string) => pickField(c, key, locale),
    [c, locale],
  );
  ...
  const mapData = useMemo(() => {
    return {
      points: points as Array<PointData> | undefined,
  ...
  }, [points, c.besanconLabel, c.besanconImage, c.besanconUrl]);
  ...
  useEffect(() => {
    if (hoveredPoint) {
      const handleClickOutside = (e: MouseEvent) => {
        ...
        if (
          !target.closest("[data-origins-map]") &&
          !target.closest("[data-origins-tooltip]")
        ) {
  ...
  useEffect(() => {
    ...
    const gMasked = svg.append("g").attr("mask", maskId);
    ...
    d3.json("/europe.geojson")
              ...
              svg
                ...
                .on("mouseenter", function () {
                  ...
                  if (
                    ...
                    const pointTitle = pickField(
                      point as Record<string, unknown>,
                      "title",
                      locale,
                    );
                    const pointDescription = pickField(
                      point as Record<string, unknown>,
                      "description",
                      locale,
                    );
      ...
      .catch(() => {
        // Silently handle errors - the map will simply not render the Europe layer
      });
  }, [dimensions, mapData, locale, p]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-base sm:text-xl text-foreground max-w-3xl mx-auto leading-relaxed">
            {p("description") ||
              "Située au cœur de la Franche-Comté, notre cuisine puise son inspiration dans les richesses du terroir local et des régions voisines."}
          </p>
        </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-sm sm:text-lg text-muted-foreground leading-relaxed max-w-2xl mx-auto">
            {p("subtitle") ||
              "De la Suisse à la Bourgogne, en passant par le Jura, nous sélectionnons les meilleurs produits pour vous offrir une expérience gastronomique unique."}
          </p>
        </div>
      </div>
      ...
      {hoveredPoint &&
        ...
        createPortal(
          <div
            ...
            className={
              hoveredPoint.url
                ? "pointer-events-auto fixed z-[9999] w-64 overflow-hidden rounded-lg border border-primary/30 bg-card shadow-2xl cursor-pointer"
                : "pointer-events-none fixed z-[9999] w-64 overflow-hidden rounded-lg border border-primary/30 bg-card shadow-2xl"
            }
