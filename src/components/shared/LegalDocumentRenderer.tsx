...
const components = {
  Heading: ({ level, children }: { level: number; children: React.ReactNode }) => {
    ...
    return <Tag className={classes[level] ?? 'text-base text-primary mt-6 mb-2 font-semibold font-display'}>{children}</Tag>;
  },
  ...
  List: ({ ordered, children }: { ordered: boolean; children: React.ReactNode }) => {
    ...
    return <Tag className={`${listClass} list-outside pl-6 mb-5 text-foreground/75 space-y-2`}>{children}</Tag>;
  },
...
export function LegalDocumentRenderer({ document }: { document?: { node?: unknown } }) {
  ...
  const content = Markdoc.transform(document.node as import('@markdoc/markdoc').Node, config);
