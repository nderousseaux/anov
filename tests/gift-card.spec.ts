...
test.describe('Système Bons Cadeaux - Page de succès /cheques-cadeaux/succes', () => {
  ...
  test('Affiche le message de confirmation de paiement', async ({ page }) => {
    await page.goto('/cheques-cadeaux/succes?session_id=test_session_123&lang=fr');
    await page.waitForTimeout(3000);
    // Vérifier que la confirmation est affichée
    await expect(page.locator('svg.w-20.h-20.text-green-500')).toBeVisible();
    await expect(page.getByText('Votre paiement a été traité avec succès')).toBeVisible();
  });
  ...
  test('Affiche une erreur si session_id manquant', async ({ page }) => {
    ...
    await expect(page.getByText("Une erreur s'est produite")).toBeVisible({ timeout: 10000 });
  });
  ...
  test('Affiche la confirmation de paiement', async ({ page }) => {
    ...
    await expect(page.getByText('Un email contenant le chèque cadeau a été envoyé au destinataire')).toBeVisible();

    // Vérifier les boutons d'action
    await expect(page.getByRole('button', { name: "Retour à l'accueil" })).toBeVisible();
    await expect(page.getByRole('button', { name: 'Acheter un autre chèque cadeau' })).toBeVisible();
  });
  ...
  test('Redirection vers la page d\'achat si erreur', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Retour aux chèques cadeaux' })).toBeVisible({ timeout: 15000 });
  });
});
...
test.describe('Interface Admin - Page Bons Cadeaux', () => {
  ...
  test('Le modal peut être fermé avec Échap', async ({ page }) => {
    ...
    await expect(page.locator('h2:text("Créer un bon cadeau")')).not.toBeVisible();
  });

  test('Le modal peut être fermé en cliquant sur le bouton X', async ({ page }) => {
    await page.getByRole('button', { name: 'Créer un bon cadeau' }).click();
    await expect(page.locator('h2:text("Créer un bon cadeau")')).toBeVisible();
  ...
  test('La fermeture du modal réinitialise le formulaire', async ({ page }) => {
    ...
    await page.keyboard.press('Escape');
    await expect(page.locator('h2:text("Créer un bon cadeau")')).not.toBeVisible({ timeout: 5000 });
    await page.getByRole('button', { name: 'Créer un bon cadeau' }).click();
    await expect(page.locator('h2:text("Créer un bon cadeau")')).toBeVisible({ timeout: 5000 });
  ...
  test('Validation du montant requis', async ({ page }) => {
    await page.getByRole('button', { name: 'Créer un bon cadeau' }).click();
    await expect(page.getByRole('button', { name: 'Créer le bon' })).toBeDisabled();
    await page.locator('input#amount').fill('50');
    await expect(page.getByRole('button', { name: 'Créer le bon' })).toBeEnabled();
  ...
  test('Fermeture du modal via clic outside', async ({ page }) => {
    await page.getByRole('button', { name: 'Créer un bon cadeau' }).click();
    await expect(page.locator('h2:text("Créer un bon cadeau")')).toBeVisible();
    await page.locator('.fixed.inset-0.z-50').click({ position: { x: 50, y: 50 } });
    await expect(page.locator('h2:text("Créer un bon cadeau")')).not.toBeVisible();
  });
});
...
test.describe('Interface Admin - Filtres', () => {
  ...
  test('Le filtre par code s\'affiche', async ({ page }) => {
    await expect(page.locator('input[placeholder="Rechercher par code..."]')).toBeVisible();
  });

  test('Le filtre par email s\'affiche', async ({ page }) => {
    await expect(page.locator('input[placeholder="Rechercher par email..."]')).toBeVisible();
  });

  test('Le bouton de réinitialisation s\'affiche quand filtres actifs', async ({ page }) => {
    await page.locator('input[placeholder="Rechercher par code..."]').fill('TEST');
    await expect(page.getByRole('button', { name: 'Réinitialiser' })).toBeVisible({ timeout: 5000 });
  });
  ...
  test('La réinitialisation des filtres fonctionne', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder="Rechercher par code..."]')).toHaveValue('', { timeout: 10000 });
  });
...
test.describe('Interface Admin - Statistiques', () => {
  ...
  test('La carte "Total émis" s\'affiche', async ({ page }) => {
    await page.waitForTimeout(1000);
    await page.locator('.grid .bg-card').first().locator('.text-2xl').first().isVisible();
  });
...
test.describe('Interface Admin - Cartes de bons cadeaux', () => {
  ...
  test('Les cartes affichent le montant', async ({ page }) => {
    ...
    const count = await cards.count();
    if (count > 0) {
      await expect(cards.first().locator('.text-lg.font-bold').first()).toBeVisible();
    }
  });
  ...
  test('Les cartes affichent le statut', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const statusText = await page.getByText(/En cours de paiement|Actif|Utilisé|Expiré/).first().textContent();
      expect(statusText).toBeDefined();
      expect(statusText).toMatch(/En cours de paiement|Actif|Utilisé|Expiré/);
    }
  });

  test('Le bouton Marquer comme utilisé s\'affiche pour ACTIVE et EXPIRED', async ({ page }) => {
    await page.waitForTimeout(1000);
    // Vérifier qu'au moins une carte a le statut ACTIVE ou EXPIRED
    const hasActiveCard = await page.locator('text=Actif|Expiré').first().isVisible({ timeout: 5000 }).catch(() => false);
    if (hasActiveCard) {
      const cards = page.locator('[data-testid="gift-card-card"]');
      const count = await cards.count();
      if (count > 0) {
        ...
        await expect(card.locator('button[title*="Marquer comme utilisé"]')).toBeVisible({ timeout: 2000 });
      }
    }
  });

  test('Le bouton Remettre à non utilisé s\'affiche pour USED', async ({ page }) => {
    await page.waitForTimeout(1000);
    // Vérifier qu'au moins une carte a le statut USED (via le texte "Utilisé")
    const hasUsedCard = await page.locator('text=Utilisé').first().isVisible({ timeout: 5000 }).catch(() => false);
    if (hasUsedCard) {
      ...
      if (count > 0) {
        ...
        await page.waitForTimeout(200);
        await expect(card.locator('button[title="Marquer comme non utilisé"]')).toBeVisible({ timeout: 2000 });
      }
    }
  });
  ...
  test('La suppression est désactivée', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(cards.first().locator('button[title="Supprimer le bon"]')).not.toBeVisible({ timeout: 2000 });
    }
  });
});
...
test.describe('Interface Admin - Actions sur les bons', () => {
  ...
  test('Marquer un bon comme utilisé', async ({ page }) => {
    ...
    if (count > 0) {
      await cards.first().hover();
      await page.locator('button[title*="Marquer comme utilisé"]').first().click();
  ...
  test('Remettre un bon USED à non utilisé', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      if (hasUsedCard) {
        ...
        await page.locator('button[title="Marquer comme non utilisé"]').click({ force: true });
        await page.waitForTimeout(1000);
      }
    }
  });
...
test.describe('Interface Admin - Navigation Header', () => {
  ...
  test('Le lien Bons Cadeaux est actif', async ({ page }) => {
    await loginToAdmin(page);
    await page.getByRole('link', { name: 'Bons Cadeaux' }).click();
    await expect(page.getByRole('link', { name: 'Bons Cadeaux' })).toHaveClass(/bg-primary\/15/, { timeout: 5000 });
  });
